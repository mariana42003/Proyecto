...
function A_historial() {
  ...
  const actualizarEstado = async (nuevoEstado) => {
    ...
    try {
      ...
      // SALIDA DE MATERIAL: cuando se CONFIRMA por primera vez un pedido
      // hecho por un cliente (origen "cliente"), se debe generar la
      // salida real de stock para el jefe — mismo mecanismo que usa
      // M_salida.jsx (fila en "movimientos" con tipo "salida" +
      // descuento de stock_actual en "productos"). Se hace aquí, al
      // confirmar, y no desde la sesión del cliente al comprar, por dos
      // razones: 1) las políticas RLS de "productos"/"movimientos" solo
      // dejan escribir a jefe/administrador, así que desde el cliente
      // fallaría siempre; 2) no tiene sentido descontar stock de un
      // pedido que el jefe todavía podría rechazar.
      // Se compara contra el estado ANTERIOR (ordenSeleccionada.estado)
      // para no descontar dos veces si se vuelve a guardar una orden
      // que ya estaba confirmada.
      const esConfirmacionNueva = nuevoEstado === "confirmada" && ordenSeleccionada.estado !== "confirmada";
      if (esConfirmacionNueva && ordenActualizada.origen === "cliente") {
        try {
          const { data: productosInventario } = await supabase
            .from("productos")
            .select("id, nombre, stock_actual");

          const productoInventario = (productosInventario || []).find(
            (p) => (p.nombre || "").trim().toLowerCase() === (ordenActualizada.producto_nombre || "").trim().toLowerCase()
          );

          if (productoInventario) {
            const nuevoStock = Math.max(0, (productoInventario.stock_actual || 0) - (ordenActualizada.cantidad || 0));
            await supabase.from("productos").update({ stock_actual: nuevoStock }).eq("id", productoInventario.id);
          }

          await supabase.from("movimientos").insert({
            producto_id: productoInventario ? productoInventario.id : null,
            tipo: "salida",
            cantidad: ordenActualizada.cantidad,
            motivo: `Venta en línea · Pedido ${ordenActualizada.codigo} · ${ordenActualizada.producto_nombre}`,
            responsable: `Confirmado a ${ordenActualizada.cliente_nombre || "cliente"}`,
          });
        } catch (errorSalida) {
          // No se revierte la confirmación de la orden si esto falla;
          // solo se deja constancia para que el jefe registre la salida
          // manualmente desde Movimientos > Salidas si hace falta.
          console.error("No se pudo registrar la salida de material de este pedido:", errorSalida);
        }
      }
