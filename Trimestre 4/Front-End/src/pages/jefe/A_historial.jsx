...
// Igual que en Stock: se muestran pocas órdenes por "pestaña" (página).
...
function A_historial() {
  ...
  // Paginación (mismo esquema que S_stock.jsx). Si al actualizarse en
  // tiempo real quedan menos páginas, se ajusta sola la página actual.
  ...
  // Ventana de hasta 5 números de página para que no crezca sin límite.
  const inicioVentana = Math.max(1, Math.min(paginaSegura - 2, totalPaginas - 4));
  const numerosPagina = Array.from(
    { length: Math.min(5, totalPaginas) },
    (_, indice) => inicioVentana + indice
  );
  const desde = filtradas.length === 0 ? 0 : (paginaSegura - 1) * REGISTROS_POR_PAGINA + 1;
  const hasta = Math.min(paginaSegura * REGISTROS_POR_PAGINA, filtradas.length);
  ...
  const actualizarEstado = async (nuevoEstado) => {
    ...
    try {
      ...
      // NOTA: la salida de material (descuento de stock + fila en "movimientos")
      // ya NO se genera aquí. Ahora la hace la base de datos apenas el cliente
      // confirma su compra (trigger en salida-automatica.sql). Si el jefe rechaza
      // el pedido, otro trigger devuelve el stock.
  ...
  return (
    <>
      <section className="panelTabla">
        ...
        <div className="ordenes-paginacion">
          <span className="ordenes-paginacion__info">
            {filtradas.length === 0 ? "Sin órdenes" : `Mostrando ${desde}–${hasta} de ${filtradas.length}`}
          </span>
          <div className="ordenes-paginacion__botones">
            <button
              type="button"
